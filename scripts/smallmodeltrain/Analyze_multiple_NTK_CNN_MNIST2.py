...
train_x = train_x.to('cuda').reshape(-1,1,28,28)
train_y = train_y.to('cuda')

test_x = test_x.to('cuda').reshape(-1,1,28,28)
...
for SEED in range(0,100):
    ...
    plt.title('MNIST2 CNN SEED {} Loss History'.format(SEED))
    #plt.savefig('./images/CNN_MNIST2_CI_trainhistory/loss_{}'.format(SEED))
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.show()
    
    plt.plot(outer_acc)
    plt.title('MNIST2 CNN SEED {} Train Acc History'.format(SEED))
    #plt.savefig('./images/CNN_MNIST2_CI_trainhistory/acc_{}'.format(SEED))
    plt.xlabel('Epoch')
    plt.ylabel('Train Acc')
    plt.show()
            
    torch.save(model.state_dict(), '../MODELS/smallmodel/MNISTCNN/MNIST_trained_SEED{}.pt'.format(SEED))
...
for SEED in range(100):
    model = make_model(SEED)
    #model.cuda()
    model.load_state_dict(torch.load('../MODELS/smallmodel/MNISTCNN/MNIST_trained_SEED{}.pt'.format(SEED),map_location=torch.device('cuda')))
    break
    ...
    np.save('../KERNELS/smallmodel/MNISTCNN/MNIST_NTK{}.npy'.format(SEED),NTK_sum.cpu().numpy())
...
for SEED in tqdm(range(0,100)):
    model.load_state_dict(torch.load('../MODELS/smallmodel/MNISTCNN/MNIST_trained_SEED{}.pt'.format(SEED)))
...
#     del evalK
#     np.save('/rcfs/projects/task0_pmml/MODELS/CIFAR2_many_model_CNN/NTK/CIFAR2_NTK{}.npy'.format(SEED),NTK.cpu().numpy())
#     del NTK
    NTK = np.load('../KERNELS/smallmodel/MNISTCNN/MNIST_NTK{}.npy'.format(SEED))
...
np.save('../experimentresults/MNIST2_CNN_100_models_results.npy',report)
