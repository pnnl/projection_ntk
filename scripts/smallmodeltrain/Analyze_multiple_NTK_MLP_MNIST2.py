...
import torchntk.autograd as torchntk
...
test_x = torch.cat([test_x_6[0:980],test_x_9[0:980]])
test_y = torch.cat([test_y_6[0:980],test_y_9[0:980]])
...
class Model(torch.nn.Module):
    ...
    def forward(self,x,):
        ...
        y = F.relu(self.l1(y)/np.sqrt(100))
        y = F.relu(self.l2(y)/np.sqrt(100))
        y = F.relu(self.l3(y)/10)
        y = self.l4(y)
        return y
...
test_loader = DataLoader(TensorDataset(test_x,test_y),batch_size=4,shuffle=False)
...
def calculate_test_acc(model,test_loader):
    with torch.no_grad():
        ...
        for data,label in test_loader:
            ...
            output = model(data)[:,0]
            acc = torch.sum(torch.sign(label.squeeze()-0.5)==torch.sign(output.squeeze()-0.5))
            accuracies.append(acc.item())
            
        print('Test Acc: {:.4f}'.format(np.sum(accuracies)/len(test_x)))
...
for SEED in range(0,100):
    ...
    for step in tqdm(range(1,N_EPOCHS+1)):       
        ...
        for data,label in train_loader:
            ...
            acc = torch.sum(torch.sign(target.squeeze()-0.5)==torch.sign(output.squeeze()-0.5))
            accuracies.append(acc.item())
    ...
    plt.title('MNIST2 MLP SEED {} Loss History'.format(SEED))
    #plt.savefig('./images/MLP_MNIST2_CI_trainhistory/loss_{}'.format(SEED))
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.show()
    
    plt.plot(outer_acc)
    plt.title('MNIST2 MLP SEED {} Train Acc History'.format(SEED))
    #plt.savefig('./images/MLP_MNIST2_CI_trainhistory/acc_{}'.format(SEED))
    plt.xlabel('Epoch')
    plt.ylabel('Train Acc')
    plt.show()
            
    torch.save(model.state_dict(), '../MODELS/smallmodel/MNISTMLP/MNIST_trained_SEED{}.pt'.format(SEED))
...
def calculate_COSSIM(NTK,):
    K0 = NTK[1960::,1960::] #this is the train data gram matrix
    K1 = NTK[0:1960,1960::] #this is the test-data's sim to train data
    K2 = NTK[0:1960,0:1960]
...
for SEED in range(100):
    model = make_model(SEED,'cuda')
    #model.cuda()
    model.load_state_dict(torch.load('../MODELS/smallmodel/MNISTMLP/MNIST_trained_SEED{}.pt'.format(SEED),map_location=torch.device('cuda')))
    
    NTK = torchntk.vmap_ntk_loader(model,combined_loader)
    NTK_sum = 0
    for key in NTK.keys():
        NTK_sum+=NTK[key]
        
    np.save('../KERNELS/smallmodel/MNISTMLP/MNIST_NTK{}.npy'.format(SEED),NTK_sum.cpu().numpy())
...
for SEED in tqdm(range(0,100)):
    model.load_state_dict(torch.load('../MODELS/smallmodel/MNISTMLP/MNIST_trained_SEED{}.pt'.format(SEED)))
    ...
    NTK = np.load('../KERNELS/smallmodel/MNISTMLP/MNIST_NTK{}.npy'.format(SEED))
...
for i in range(100):
    for j in range(100):
        if i==j or j<=i or Ws[i]==0 or Ws[j]==0:
            continue
        else:
...
for i in range(100):
    if Omegas[i]==0 or Ws[i]==0:
        continue #this is a case our definition of Gamma diverges
...
report['SEED'] = SEEDs
report['overlap_misclassified_kSVM_models'] = overlap_misclassified_kSVM_models
...
np.save('./experimentresults/MNIST2_MLP_100_models_results.npy',report)
